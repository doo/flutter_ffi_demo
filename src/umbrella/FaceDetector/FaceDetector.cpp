//
// Created by Serhii Chaban on 19.10.21.
//
// implementation of the face detector was taken from here https://github.com/bewagner/visuals
#include <sstream>
#include <vector>
#include <string>
...
FaceDetector::FaceDetector(const char *faceDetectionConfiguration,
                           const char *faceDetectionWeights) : confidence_threshold_(0.5),
...
// Note: The varibles MODEL_CONFIGURATION_FILE and MODEL_WEIGHTS_FILE are passed in via cmake
    network_ = cv::dnn::readNetFromCaffe(faceDetectionConfiguration, faceDetectionWeights);

    if (network_.empty()) {
        std::ostringstream ss;
        ss << "Failed to load network with the following settings:\n"
           << "Configuration: " + std::string(faceDetectionConfiguration) + "\n"
           << "Binary: " + std::string(faceDetectionWeights) + "\n";
